...
### (Optional) Installation chunk

```{r install, eval=FALSE}
install.packages(c(
  "textdata", "dplyr", "stringr", "janeaustenr", "ggplot2",
  "gutenbergr", "scales", "tidyr", "tidytext",
  "RColorBrewer", "wordcloud", "reshape2", "forcats", "tibble"
))

# If gutenbergr from CRAN gives mirror/download issues, install the dev version:
# remotes::install_github("ropensci/gutenbergr")
```
...
Because we've been using tidy tools, our word counts are stored in a tidy data frame. This allows us to pipe this directly to the `ggplot` function and to create a visualization of the most common words.
...
Now let's calculate the frequency for each word for the works of the three authors by binding the data frames together. We can use `pivot_wider()` and `pivot_longer()` from `tidyr` to reshape our data frame so that it is just what we need for plotting and comparing the three sets of novels.
...
frequency <- bind_rows(
  ...
  pivot_longer(
    cols = c(`Brontë Sisters`, `Charles Dickens`),
    names_to = "author",
...
Let's make the plot. Note that we surround the `Jane Austen` column name with backticks since it is a non-standard variable name - it has a space between the two names, which R doesn't like. You could get around this by naming the variable `Jane_Austen`, for example. 
...
```{r}
cor.test(
  data = frequency[frequency$author == "Charles Dickens", ],
  ~ proportion + `Jane Austen`
)
```
...
Let’s look at the words with a joy score from the NRC lexicon. What are the most common joy words in one of Austen’s novels? First, we need to take the text of the novels and convert the text to the tidy format using `unnest_tokens()`. Let’s also set up some other columns to keep track of which line and chapter of the book each word comes from. We use `group_by` and `mutate` to construct those columns.

> Note: here we recreate `tidy_books` from the raw text so that it includes **all words** (not just after stop-word removal), which is useful for sentiment analysis.
...
Now that the text is in a tidy format with one word per row, we are ready to do the sentiment analysis. First, let's use the NRC lexicon and `filter()` for the joy words. Next, let's `filter()` the data frame with the text from the books for the words from *Mansfield Park* and then use `inner_join()` to perform the sentiment analysis. What are the most common joy words? Let’s use `count()` from dplyr.
...
## Public Health Example: Free-text Survey Responses

To connect these tools to public health practice, let's look at a tiny synthetic example that mimics free-text responses from a patient experience survey at a community clinic. In practice, you might have hundreds or thousands of responses; here we keep it small so you can see what's happening.

```{r}
ph_comments <- tibble(
  id = 1:6,
  response = c(
    "The clinic hours make it hard to come in after work.",
    "The staff were very kind and explained the vaccine side effects clearly.",
    "I am worried about sharing my immigration status when I register.",
    "The wait time was too long and the waiting room felt crowded.",
    "I appreciated that the nurse spoke my language and listened to my concerns.",
    "It was confusing to get an appointment through the new online portal."
  )
)
...
### Tidy the survey text

We convert the responses to one-token-per-row and remove stop words, just as we did for the novels.
...
### Sentiment for each response

We can get a rough sense of whether each comment is more positive or negative using the `bing` lexicon. We'll compute a **net sentiment** score for each response: number of positive words minus number of negative words.

```{r}
ph_sentiment <- ph_tidy %>%
  inner_join(get_sentiments("bing"), by = "word") %>%
  count(id, sentiment) %>%
  pivot_wider(names_from = sentiment, values_from = n, values_fill = 0L) %>%
...
* Look at how sentiment varies by clinic site, language, or time period.
* Flag very negative comments for qualitative review by the quality improvement team.

### Which words characterize each response? (tf-idf)

We can also use tf-idf to identify which words are **most distinctive** for each response. Here, each response `id` is treated as a “document” in the corpus.
...
Let's visualize the top words per response:
...
ph_tf_idf %>%
  ...
  ungroup() %>%
  ggplot(aes(tf_idf, fct_reorder(word, tf_idf), fill = factor(id))) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~id, ncol = 2, scales = "free_y") +
  labs(
    x = "tf-idf (distinctive importance within this response)",
    y = NULL
  )
```

In a real-world public health setting, this type of plot can help you see:

* Which themes show up repeatedly (e.g., *wait*, *crowded*, *online portal*).
* Which concerns might reflect **access** issues (hours, portal usability).
* Which comments reflect **trust and communication** (language, listening, explaining side effects).

From here, the same workflows you used on novels can be applied to:

* Open-ended survey questions (e.g., CHNA, patient experience).
* Provider notes or triage notes (with appropriate privacy safeguards).
* Public health reports, guidelines, or policy documents.

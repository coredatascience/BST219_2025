...
## TLDR: functions covered

* From the `lubridate` package: `mdy`, `myd`, `dmy`, `dym`, `ydm`, `ymd`, `hms`, `mdy_hms`, `myd_hms`, `dmy_hms`, `dym_hms`, `ydm_hms`, `ymd_hms`
...
The `lubridate` package is a member of the Tidyverse, designed to simplify and streamline the process of working with dates and times. It provides a user-friendly and consistent set of functions for parsing, manipulating, and performing calculations on date-time objects.
...
1. Make the following strings dates.
...
2. Create a data frame with columns `clean_dates`, `month`, `day` and `year` from the string of dates in question 1.

```{r}
# Your code here
```

3. Join the data frame you just created with the one below using `full_join`. Are there any missing values? 

```{r}
df2 <- data.frame(
  year = c(2003, 2009, 2012, 2016),
	age = c(17, 23, 26, 30))
...
new_df <- full_join(clean_dates, df2, by = "year")
new_df
```

Yes, there are two missing values in the month and day columns, and one missing value in the age column. 

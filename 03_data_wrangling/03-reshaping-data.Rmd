...
### TLDR: reshaping functions covered

* `pivot_longer()`: when columns are values of a variable (many columns → two columns: key & value).
* `pivot_wider()`: when a key column should become new columns (two columns → many columns).
* `separate()`: split one column into many; be explicit about seperator and consider extra/fill.
* `unite()`: combine multiple columns into one; control seperator and whether to remove originals.
...
1. Using the `gapminder` dataset, keep `country`, `year`, `life_expectancy`, and `infant_mortality`. Convert the data frame to long format with a column called `metric` with values that are either `infant_mortality` or `life_expectancy`, and another column called `value` that stores the value of `infant_mortality` and `life_expectancy` for each country and year. 
...
gap_long<- gapminder %>%
  select(country, year, life_expectancy, infant_mortality) %>%
  pivot_longer(-c(country, year), names_to = "metric", values_to = "value")
head(gap_long)
```

2. Take the long format data frame you just created in question 1 and put it back into wide format so that `life_expectancy` and `infant_mortality` become separate columns again. 
...
3. Using the `murders` dataset, combine `state` and `abb` into a new column called `state_abb` with values that combine the values in the state and abb columns with a hyphen in the middle. For example, `Massachusetts-MA` should be a value in the `state_abb` column. 

```{r}
# Your code here
```

4. Using the data frame you created in question 3, split `state_abb` back into a state name column called `state_name` and a state abbreviation column called `abb2`. Verify that the `state` column matches the `state_name` column and the `abb` column matches the `abb2` column. 
...
```{r}
gm2 <- gapminder %>%
  select(country, continent, year, life_expectancy, infant_mortality)

gm_long <- gm2 %>%
  pivot_longer(
    cols = c(life_expectancy, infant_mortality),
    names_to = "metric",
    values_to = "value"
  )

gm_long %>% 
  arrange(country, year, metric) %>% 
  head(10)
```
...
```{r}
mur2 <- murders %>%
  unite("state_abb", state, abb, sep = "-", remove = FALSE)
head(mur2)
```
...
```{r}
mur_sep <- mur2 %>%
  separate(state_abb, into = c("state_name", "abb2"), sep = "-")

# Check that abb2 matches the original abb
all(mur_sep$abb2 == mur_sep$abb)   # should return TRUE

head(mur_sep %>% select(state, abb, state_name, abb2))
```

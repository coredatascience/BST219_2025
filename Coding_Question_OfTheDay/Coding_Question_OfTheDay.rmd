...
avg <- gapminder %>%
  group_by(year) %>%
  dplyr::summarize(global_avg = mean(life_expectancy, na.rm=TRUE)) 
print(avg)
```
...
gapminder %>% 
  ggplot() +
  geom_line(aes(x = year, y = life_expectancy, group = country),
            color = 'darkgrey', 
            alpha = 0.4) +
  geom_line(data = avg, 
            mapping = aes(x = year, y = global_avg), 
            linewidth = 1) +
  xlab("Year") +
  ylab("Life expectancy (Years)") +
  ggtitle("Life expectancy across countries") +
  #annotate(geom = "text", x = 1970, y = 54, 
...
gapminder <- gapminder %>%
  filter(year == 1999) %>%
  mutate(life_expectancy_category = 
           case_when(
  ...
  mutate(life_expectancy_category = 
           factor(life_expectancy_category, 
           levels = c("Low", "Medium", "High")
          )
         )
```

Another (more concise) way to code the above code chunk:
...
```{r}
gapminder %>% 
  ggplot(aes(life_expectancy_category)) +
  geom_bar(color = "black", fill = "steelblue") +
  labs(x = "Life expectancy category",
...
Using the `gapminder` dataset, create a new data frame with only the `country`, `year`, `life_expectancy`, and `fertility` columns. Use the `pivot_longer()` function to reshape the dataset so that the `life_expectancy` and fertility columns are combined into a single value column called `value`. The column names should be stored in a new column called `health_indicator`.
...
Below, a link to a dataset available on GitHub is provided. The data comes from the [Johns Hopkins University COVID-19 data repository](https://github.com/CSSEGISandData/COVID-19/tree/master).  This particular dataset contains reported COVID-19 case counts from countries around the globe from  January 22, 2020 to March 23, 2020. 

Read in the data using the code provided. This dataset is currently in wide format. Convert the data frame to long format, with a column named `date` and a column named `cases`. Then, make the date column values into date objects. 

Bonus challenge: rename the `Country/Region` column country. Create a new data frame with country name, date, and the total number of cases in the country on each day.
...
```{r}
long_data <- data %>%
  pivot_longer(cols = `1/22/20`:`3/23/20`,
               names_to = "date",
               values_to = "cases") %>%
...
```{r}
data_long_country <- long_data %>% 
  rename(country = "Country/Region") %>% 
  group_by(country, date) %>%
  dplyr::summarize(cases = sum(cases)) %>%
  ungroup()

head(data_long_country)
```
...
Code from 11/4 question of the day:

...
## TLDR: ggplot2 and plot components

In this module, you’ll learn how to make clear and effective data visualizations in R with ggplot2.

**Core idea**: ggplot2 is built on the grammar of graphics → plots are constructed by combining data, aesthetics (`aes()`), and geometric layers (`geom_*`).

**Core functions**:

* `ggplot(data, aes(x, y))` – initialize a plot with data and mappings
* `geom_point()`, `geom_bar()`, `geom_boxplot()` – common plot types
* `labs(title = , x = , y = )` – add labels
* `theme_minimal()`, `theme_bw()` – adjust themes
* `scale_x_*()`, `scale_y_*()`, `scale_color_*()` – fine-tune scales and colors
	
**Main points**:

1. Start with data + aesthetics + geom to build any plot.
2. Add layers to customize appearance and improve clarity.
3. Always design visualizations with readability and reproducibility in mind.
...
We will be using the [Framingham Heart Study](https://search.r-project.org/CRAN/refmans/riskCommunicator/html/framingham.html) dataset that is available from the `riskCommunicator` package in R. 

```{r}
# Check if the package is installed
if (!requireNamespace("riskCommunicator", quietly = TRUE)) {
...
1. Create a scatterplot of systolic blood pressure (`SYSBP`) and age (`AGE`), colored by sex (`SEX`).

```{r}
# Your code here
```

2. Change the x and y axes and legend title and legend group labels to be more informative. 

```{r}
# Your code here 
```

3. A lot of the points overlap and making the points more transparent might help with interpretation. Make the points more transparent and see if it helps with interpretability. If it doesn't what do you think would be helpful?
...
```{r}
framingham %>% 
  ggplot(aes(x = AGE, y = SYSBP, color = as.factor(SEX))) +
  geom_point()
```
...
Answer: Unfortunately, in this instance, making the points more transparent does not help with interpretability - there are just too many dots. Creating two scatterplots, one for males and one for females, would be better in this case. 
